  ...
  const nav = adminOnly
    ...
    : $(`
      <nav class="tabs">
        ...
        ${canPrint && State.user?.rol !== 'operario' ? `<button data-v="imprimir" class="${State.view==='imprimir'?'active':''}">${ICON.qr}<span>Imprimir</span></button>` : ''}
        <button data-v="perfil" class="${State.view==='perfil'   ?'active':''}">${ICON.user}<span>Perfil</span></button>
        <button class="nav-logout" id="nav-logout-btn">${ICON.logout}<span>Cerrar sesión</span></button>
      </nav>
    `);
...
export function renderScanView() {
  ...
  wrap.innerHTML = `
    ...
    <div class="section" style="padding-top:0; padding-bottom:0; display:flex; flex-direction:column; gap:10px;">
      ...
      <button class="action-card" id="btn-inv-ubicacion" style="background:linear-gradient(135deg,rgba(34,197,94,0.12) 0%,rgba(34,197,94,0.04) 100%);border-color:rgba(34,197,94,0.35);">
        ...
        <div class="action-card-text">
          <div class="action-card-title">Inventario en ubicación</div>
          <div class="action-card-sub">Cajas + productos sueltos en una posición</div>
        </div>
  ...
  wrap.querySelector('#btn-inv-ubicacion')?.addEventListener('click', () => {
    if (scannerActive()) stopScanner();
